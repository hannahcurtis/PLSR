...
df_all_ponds <- read.csv("/Users/hannahcurtis/Desktop/School/UWMadison/Research/Data Analysis/Paper 1 Modeling/all_predictor_vars.csv")
...
plsr_model <- plsr(Attenuation ~Max.Bounce.Norm+Decline.Rate.Norm+Inflow.Rate.Norm+Temp.Variance.Norm+Contant.Water.Level.6in+Outlet.Water.Level+Below.Outlet.Water.Level+Water.Level.25.75.Quantile+Pond.Skewness+Pond.Area.ft2+WA.PA+Res.Time+Avg.Pond.Depth+Max.Pond.Depth+L.to.W.ratio+HF.Outlet.Restrict.1+Number.of.Outlets+Total...Veg.Species+X..Non.native.species.1+Watershed.Area.ft2+Impervious.Percent.Recalc+Residential.Percent+Urban.Baseflow+Average.Watershed.Slope+Canopy.Percent+avg_bd..g.cm3.+avg_sed_cm+per_om+Runoff.Depth.Norm+Comm.Percent.200m.Buffer+Res.Percent.200m.Buffer+GS.Percent.200m.Buffer+Mow.Distance, data = train_data, ncomp = 4, validation = "CV", segments=5)

# Predict on validation data
predictions <- predict(plsr_model, newdata = validation_data, ncomp = 4)
predictions <- as.numeric(predictions)

# Calculate performance metrics
actual_values <- validation_data$Percent.Days.Outflow
...
plot(actual_values, predictions, xlab = "Actual", ylab = "Predicted", main = "Predicted vs. Actual Percent Days Outflow", pch = 16, col = "dodgerblue2", bg="transparent", cex.axis=1)
grid()
# Add the line y = x
...
# run PLSR
model <- plsr(SPC.Difference ~ 	Max.Bounce.Norm+Decline.Rate.Norm+Pond.Skewness+HF.Outlet.Restrict.1+X..Non.native.species.1+Runoff.Depth.Norm+GS.Percent.200m.Buffer+Mow.Distance
,
  data=df_spc, scale=TRUE, validation="CV", segments=5,  ncomp=8
)
...
plot(model, ncomp=4, line=TRUE, main="",  xlab="Measured SPC Difference", ylab="Predicted SPC Difference", pch = 16, col = "seagreen3", bg="transparent", cex.axis=1)
grid()
...
# run PLSR
model <- plsr(Attenuation ~ 	Max.Bounce.Norm+Decline.Rate.Norm+Inflow.Rate.Norm+Water.Level.25.75.Quantile+Water.Level.10.90.Quantile+Pond.Skewness+Pond.Area.ft2+WA.PA+Max.Pond.Depth+Number.of.Outlets+Impervious.Percent.Recalc+Greenspace.Percent+Curve.Number+Average.Watershed.Slope+avg_sed_cm+Runoff.Depth.Norm+Res.Percent.200m.Buffer+GS.Percent.200m.Buffer+Mow.Distance
,
